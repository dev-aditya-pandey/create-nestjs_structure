...
const program = new commander.Command(packageJson.name)
  ...
  .on('--help', () => {
    ...
    console.log(
      `      ${chalk.cyan(
        'https://github.com/dev-aditya-pandey/create-nestjs_structure/issues/new',
      )}`,
    )
    console.log()
  })
...
if (typeof projectName === 'undefined') {
  ...
  console.log(`npx ${chalk.cyan(program.name())} ${chalk.green('demo-nest')}`)
  console.log()
  process.exit(1)
}
...
function createStructure(name, verbose, useNpm, tarball = tarballURL) {
  ...
  console.log(`Creating a new Nest structure in ${chalk.green(root)}.`)
  console.log()
  ...
  run(root, appName, verbose, tarball, useYarn)
}
...
async function run(root, appName, verbose, tarball, useYarn) {
  try {
...
function checkThatNpmCanReadCwd() {
  ...
  try {
    ...
    // in a Nest process was not enough.
    childOutput = spawn.sync('npm', ['config', 'list']).output.join('')
  } catch (err) {
    // Something went wrong spawning nest.
